...
object LouvainCore {
  ...
  def louvain(sc: SparkContext, graph: Graph[VertexState, Long], minProgress: Int = 1, progressCounter: Int = 1): (Double, Graph[VertexState, Long], Int) = {
     ...
     */
    def sendMsg(triple: EdgeContext[VertexState, Long, Map[(Long, Long), Long]]) = {
      val m1 = Map((triple.srcAttr.community, triple.srcAttr.communitySigmaTot) -> triple.attr)
      val m2 = Map((triple.dstAttr.community, triple.dstAttr.communitySigmaTot) -> triple.attr)
      triple.sendToDst(m1)
      triple.sendToSrc(m2)
    }
    ...
    var msgRDD = louvainGraph.aggregateMessages[Map[(Long, Long), Long]](triplet => sendMsg(triplet), mergeMsg)
    ...
    do {
      ...
      val labeledVertices = louvainVertJoin(louvainGraph, msgRDD, totalGraphWeight, even).cache()
      ...
      val updatedVerts = labeledVertices.join(communityMapping).map({
      ...
      // gather community information from each vertex's local neighborhood
      val oldMsgs = msgRDD
      msgRDD = louvainGraph.aggregateMessages[Map[(Long, Long), Long]](triplet => sendMsg(triplet), mergeMsg).cache()
      ...
      // half of the communities can swtich on even cycles
    ...
    val newVertices = louvainGraph.vertices.innerJoin(msgRDD)((vid, vdata, msgs) => {
      ...
      val ki = vdata.nodeWeight + vdata.internalWeight
      val q = (k_i_in.toDouble / M) - ((sigmaTot * ki) / math.pow(M, 2))
  ...
  private def louvainVertJoin(louvainGraph: Graph[VertexState, Long], msgRDD: VertexRDD[Map[(Long, Long), Long]], totalEdgeWeight: Broadcast[Long], even: Boolean) = {
    louvainGraph.vertices.innerJoin(msgRDD)((vid, vdata, msgs) => {
      ...
      msgs.foreach({ case ((communityId, sigmaTotal), communityEdgeWeight) =>
        ...
        //        println("community: " + communityId + " sigma:" + sigmaTotal + " edgeWeight:" + communityEdgeWeight + "  q:" + deltaQ)
  ...
  def compressGraph(graph: Graph[VertexState, Long], debug: Boolean = true): Graph[VertexState, Long] = {
    ...
    val compressedGraph = Graph(newVertices, edges).partitionBy(PartitionStrategy.EdgePartition2D).groupEdges(_ + _)
