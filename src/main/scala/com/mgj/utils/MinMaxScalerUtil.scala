...
/**
  * Params for [[MinMaxScalerUtil]] and [[MinMaxScalerModelUtil]].
  */
private[feature] trait MinMaxScalerParamsUtil extends Params with HasInputCol with HasOutputCol {
...
/**
  ...
  */
@Experimental
class MinMaxScalerUtil(override val uid: String)
  extends Estimator[MinMaxScalerModelUtil] with MinMaxScalerParamsUtil {
  ...
  override def fit(dataset: DataFrame): MinMaxScalerModelUtil = {
    transformSchema(dataset.schema, logging = true)
    val input = dataset.select($(inputCol)).map { case Row(v: Vector) => v }
    val summary = Statistics.colStats(input)
    copyValues(new MinMaxScalerModelUtil(uid, summary.min, summary.max).setParent(this))
  }
  ...
  override def copy(extra: ParamMap): MinMaxScalerUtil = defaultCopy(extra)
}

/**
  * :: Experimental ::
  * Model fitted by [[MinMaxScalerUtil]].
  *
  ...
  */
@Experimental
class MinMaxScalerModelUtil private[ml](
                                     override val uid: String,
                                     val originalMin: Vector,
                                     val originalMax: Vector)
  extends Model[MinMaxScalerModelUtil] with MinMaxScalerParamsUtil {
  ...
  override def transform(dataset: DataFrame): DataFrame = {
    ...
    val reScale = udf { (vector: Vector) =>
      ...
      var i = 0
      while (i < size) {
        val raw = if (originalRange(i) != 0) (values(i) - minArray(i)) / originalRange(i) else 0
        values(i) = raw * scale + $(min)
        i += 1
      }
      Vectors.dense(values).toSparse
    }
  ...
  override def copy(extra: ParamMap): MinMaxScalerModelUtil = {
    val copied = new MinMaxScalerModelUtil(uid, originalMin, originalMax)
