...
object SampleV2Util {
  ...
  def getOrderSample(sqlContext: HiveContext, bizdate: String, appIds: String*): DataFrame = {
    ...
    val sampleFinalDF = sqlContext.createDataFrame(orderSampleRDD.map(x => Row(x._1, x._2, sdf.format(x._3.toLong * 1000), Math.log(1 + x._4.toDouble).toString, x._5)), schema).cache()
  ...
  def getClickSample(sqlContext: HiveContext, bizdate: String, appIds: String*): DataFrame = {
    ...
    val sdf: SimpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
    val sampleFinalDF = sqlContext.createDataFrame(sampleFinal.map(x => Row(x._1, x._2, sdf.format(x._3.toLong * 1000), Math.log(1 + x._4.toDouble).toString, x._5)), schema).cache()
    return this.takeSample(sampleFinalDF)
  }
