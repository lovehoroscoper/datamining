...
object UserCategoryPrefer {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sampleOrderSql = "select user_id, category_id, time from s_dg_user_base_log where pt = '" + bizdate + "' and action_type = 'order' and platform_type = 'app'"
    val sampleSql = "select user_id, category_id, time from s_dg_user_base_log where pt = '" + bizdate + "' and action_type = 'click' and platform_type = 'app'"
    val clickSql = "select user_id, category_id, time from s_dg_user_base_log where pt >= '" + bizdateSub30 + "' and pt <= '" + bizdateSub1 + "' and action_type = 'click' and platform_type = 'app'"
    val orderSql = "select user_id, category_id, time from s_dg_user_base_log where pt >= '" + bizdateSub30 + "' and pt <= '" + bizdateSub1 + "' and action_type = 'order'"
    val addCartSql = "select user_id, category_id, time from s_dg_user_base_log where pt >= '" + bizdateSub30 + "' and pt <= '" + bizdateSub1 + "' and action_type = 'add_cart'"
    val favorSql = "select user_id, category_id, time from s_dg_user_base_log where pt >= '" + bizdateSub30 + "' and pt <= '" + bizdateSub1 + "' and action_type = 'favor'"
    ...
    //    val sampleOrderSql = "select user_id, Category_id, time from s_dg_user_base_log where pt = '2015-11-26' and action_type = 'order' and platform_type = 'app' limit 10000"
    //    val sampleSql = "select user_id, Category_id, time from s_dg_user_base_log where pt = '2015-11-26' and action_type = 'click' and platform_type = 'app' limit 10000"
    //    val clickSql = "select user_id, Category_id, time from s_dg_user_base_log where pt >= '2015-11-23' and pt <= '2015-11-25' and action_type = 'click' and platform_type = 'app' limit 10000"
    //    val orderSql = "select user_id, Category_id, time from s_dg_user_base_log where pt >= '2015-11-23' and pt <= '2015-11-25' and action_type = 'order' limit 10000"
    //    val addCartSql = "select user_id, Category_id, time from s_dg_user_base_log where pt >= '2015-11-23' and pt <= '2015-11-25' and action_type = 'add_cart' limit 10000"
    //    val favorSql = "select user_id, Category_id, time from s_dg_user_base_log where pt >= '2015-11-23' and pt <= '2015-11-25' and action_type = 'favor' limit 10000"
    ...
    //    sampleDF.registerTempTable("s_dg_user_category_prefer_sample_temp")
    //    sqlContext.sql("drop table if exists s_dg_user_category_prefer_sample")
    //    sqlContext.sql("create table s_dg_user_category_prefer_sample as select * from s_dg_user_category_prefer_sample_temp")
    sqlContext.sql("drop table if exists s_dg_user_category_prefer_sample")
    sampleDF.write.saveAsTable("s_dg_user_category_prefer_sample")

    val sampleDFOrder: DataFrame = sqlContext.createDataFrame(sampleOrderFinal, schema)
    //    sampleDFOrder.registerTempTable("s_dg_user_category_prefer_order_sample_temp")
    //    sqlContext.sql("drop table if exists s_dg_user_category_prefer_order_sample")
    //    sqlContext.sql("create table s_dg_user_category_prefer_order_sample as select * from s_dg_user_category_prefer_order_sample_temp")
    sqlContext.sql("drop table if exists s_dg_user_category_prefer_order_sample")
    sampleDFOrder.write.saveAsTable("s_dg_user_category_prefer_order_sample")
  }
}
