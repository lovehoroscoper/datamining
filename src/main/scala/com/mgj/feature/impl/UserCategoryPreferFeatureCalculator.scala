...
class UserCategoryPreferFeatureCalculator extends FeatureCalculator {
  ...
  override def getFeatureRDD(sc: SparkContext, sqlContext: HiveContext): Seq[(RDD[(String, List[String])], List[String], String)] = {
    ...
    result.addAll(temp)
    result.add(getFeature(sqlContext, itemCategoryFeatureDF, FeatureType.ITEM))
  ...
  override def toString = s"UserCategoryPreferFeatureCalculator($featureName, $userField, $itemField, $userFieldPath, $itemFieldPath, $bizDate, $tableName)"
}
