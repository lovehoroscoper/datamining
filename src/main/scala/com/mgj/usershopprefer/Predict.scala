...
object Predict {
  def main(args: Array[String]): Unit = {
    ...
    val clickSql = "select user_id, shop_id, time from s_dg_user_base_log where pt >= '" + bizdateSub30 + "' and pt <= '" + bizdate + "' and action_type = 'click' and platform_type = 'app'"
    val orderSql = "select user_id, shop_id, time from s_dg_user_base_log where pt >= '" + bizdateSub30 + "' and pt <= '" + bizdate + "' and action_type = 'order'"
    val addCartSql = "select user_id, shop_id, time from s_dg_user_base_log where pt >= '" + bizdateSub30 + "' and pt <= '" + bizdate + "' and action_type = 'add_cart'"
    val favorSql = "select user_id, shop_id, time from s_dg_user_base_log where pt >= '" + bizdateSub30 + "' and pt <= '" + bizdate + "' and action_type = 'favor'"
    ...
    HiveUtil.featureHdfsToHive(sc, sqlContext, "user_shop_prefer", userShopPerferPath, sdf.format(calendar.getTime), "s_dg_user_shop_prefer", FeatureType.USER)
    HiveUtil.featureHdfsToHive(sc, sqlContext, "user_shop_prefer_order", userShopPerferOrderPath, sdf.format(calendar.getTime), "s_dg_user_shop_prefer_order", FeatureType.USER)
  }
}
