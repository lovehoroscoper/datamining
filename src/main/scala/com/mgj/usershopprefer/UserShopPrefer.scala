...
object UserShopPrefer {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val orderSql = "select user_id, shop_id, time from s_dg_user_base_log where pt >= '" + bizdateSub30 + "' and pt <= '" + bizdateSub1 + "' and action_type = 'order'"
    val addCartSql = "select user_id, shop_id, time from s_dg_user_base_log where pt >= '" + bizdateSub30 + "' and pt <= '" + bizdateSub1 + "' and action_type = 'add_cart'"
    val favorSql = "select user_id, shop_id, time from s_dg_user_base_log where pt >= '" + bizdateSub30 + "' and pt <= '" + bizdateSub1 + "' and action_type = 'favor'"
