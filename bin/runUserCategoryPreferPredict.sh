...
hdfs dfs -test -e ${USER_CATEGORY_PREFER_HDFS_DIR}
if [ $? -eq 0 ] ;then
    echo "${USER_CATEGORY_PREFER_HDFS_DIR} exists"
    hdfs dfs -rm -r ${USER_CATEGORY_PREFER_HDFS_DIR}
fi

USER_CATEGORY_PREFER_ORDER_HDFS_DIR="/user/digu/userGBPreferOrder"
hdfs dfs -test -e ${USER_CATEGORY_PREFER_ORDER_HDFS_DIR}
if [ $? -eq 0 ] ;then
    echo "${USER_CATEGORY_PREFER_ORDER_HDFS_DIR} exists"
    hdfs dfs -rm -r ${USER_CATEGORY_PREFER_ORDER_HDFS_DIR}
fi
...
hdfs dfs -cp ${USER_CATEGORY_PREFER_HDFS_DIR} ${RECORD_PATH}/${CUR_DATE} &
...
hdfs dfs -cp ${USER_CATEGORY_PREFER_ORDER_HDFS_DIR} ${RECORD_PATH}/${CUR_DATE} &
